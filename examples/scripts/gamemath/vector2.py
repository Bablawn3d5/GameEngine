...
class Vector2(object):
    ...
    def __init__(self, x=0., y=0.):
        ...
        @type x: number
        @param x: The x value (defaults to 0.), or a container of 2 values, or with .x and .y accessors
        @type x: number
        @param y: The y value (defaults to 0.)

        """
        if hasattr(x, "x") and hasattr(x, "y"):
            self._v = [float(x.x), float(x.y)]
    ...
    def copy_to(self, rhs):
        """Copies this object to rhs with .x and .y accessors.

        @param rhs: An object with .x and .y accessors
